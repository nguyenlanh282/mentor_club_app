 ...
 * Tùy chọn ghi đè:
 *   LARK_APP_ID (qua env), LARK_APP_TOKEN (base), LARK_TABLE_ID, LARK_DOMAIN, FB_VERSION
...
const CFG = {
  ...
  APP_TOKEN:   process.env.LARK_APP_TOKEN || '', // base chứa bảng Page
...
(async()=>{
  ...
  const buildFields = (p, full) => {
    const f = full ? { [fName]: p.name, [fId]: typeOf(fId)===2 ? Number(p.id) : p.id } : {};
